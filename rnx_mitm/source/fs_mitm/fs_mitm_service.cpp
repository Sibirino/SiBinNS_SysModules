...
namespace ams::mitm::fs {
    ...
    namespace {
        ...
        constexpr const char AtmosphereHblWebContentDir[] = "/SiBin/hbl_html/";
    ...
    Result FsMitmService::OpenSdCardFileSystem(sf::Out<std::shared_ptr<IFileSystemInterface>> out) {
        ...
        std::shared_ptr<fs::fsa::IFileSystem> redir_fs = std::make_shared<fssystem::DirectoryRedirectionFileSystem>(std::make_shared<RemoteFileSystem>(sd_fs), "/Nintendo", "/Nintendo");
    ...
    Result FsMitmService::OpenSaveDataFileSystem(sf::Out<std::shared_ptr<IFileSystemInterface>> out, u8 _space_id, const fs::SaveDataAttribute &attribute) {
        ...
        FsFile flag_file;
        R_UNLESS(R_FAILED(OpenAtmosphereSdFile(&flag_file, "/SiBin/saveredirection", OpenMode_Read)), sm::mitm::ResultShouldForwardToSession());
